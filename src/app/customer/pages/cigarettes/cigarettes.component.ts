...
export class CigarettesComponent implements OnInit {
  ...
  async ngOnInit() {
    this.cigarettes = await this.appService.getProductsByCategory('cigarettes');
  }
  ...
  async addToCart(cigarettes: any) {
    try {
      ...
      const existingProduct = currentCart.some((item) => item.id === cigarettes.id);

      if (!existingProduct) {
        // Add item to Firestore cart
        await this.appService.addToCart(cigarettes);
